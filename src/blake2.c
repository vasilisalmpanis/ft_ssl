#include <ft_ssl.h>
// Spec: https://datatracker.ietf.org/doc/html/rfc7693.html#section-1
...
void blake2s_update(blake2s_data *ctx,
		const void *in, size_t inlen);

void blake2s_final(blake2s_data *ctx, void *out);

static void blake2_init(struct program_ctx *ctx)
{
	blake2s_data *data = malloc(sizeof(blake2s_data));
	size_t i = 0;
	int keylen = 0;

	if (data == NULL) error("Fatal: Out of memory");

	memset(data, 0, sizeof(blake2s_data));

	for (i = 0; i < 8; i++)             // state, "param block"
		data->h[i] = blake2s_iv[i];

	data->h[0] ^= 0x01010000 ^ (keylen << 8) ^ blake2_type.digest_size;
	data->t[0] = 0;                      // input count low word
	data->t[1] = 0;                      // input count high word
	data->c = 0;                         // pointer within buffer
...
static void blake2s_compress(blake2s_data *ctx, int last)
{
...
void blake2s_final(blake2s_data *ctx, void *out)
{
	...
	for (i = 0; i < blake2_type.digest_size; i++) {
		((uint8_t *) out)[i] =
...
static void blake2_digest(struct program_ctx *ctx, bool stdin)
{
	...
	blake2s_data *data = (blake2s_data *)ctx->data;
	blake2s_update(data, (const void *)ctx->user_input,
			(size_t)ctx->user_input_len);
	blake2s_final(data, (void *)buffer);
	print_digest(ctx, "BLAKE2S-256", buffer, stdin);
}
